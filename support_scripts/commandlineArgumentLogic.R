...
optionList = list(make_option(opt_str = c("-f", "--file"),
                  ...
                  make_option(opt_str = c("-a", "--affinity"),
                              ...
                              type = "double",
                              default = NULL,
                              help = "netMHCpan affinity cutoff (optional, default is model cutoff)"),
                  make_option(opt_str = c("-r", "--rank"),
                              action = "store",
                              type = "double",
                              default = NULL,
                              help = "netMHCpan percentile rank cutoff (optional, default is model cutoff - suggested is 2.8 ~500nM in A*02:01)"),
                  make_option(opt_str = c("-d", "--model"),
                              action = "store",
                              type = "double",
                              default = 0.02,
                              help = "Random forest prediction model cutoff (optional, default: >= %default)"),
...
if ((commandlineArguments$single | commandlineArguments$structural) & is.numeric(commandlineArguments$model)) {
	message("RNA expression data is currently needed for random forest model; not compatible with single sequence/structural variant predictions")
	q(status = 1)
